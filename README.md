...
> **Note:** It is important to note that `create-react-app` is now deprecated by the React team. Though it remains functional, developers are encouraged to use alternative tools and frameworks. Since these frameworks are beyond the scope of this course, we will continue to use `create-react-app` for our purposes.
>
...
2.  Create a `Main.js`, `Currencies.js`, and `Price.js` file in the `pages` folder.
3.  Create the component boilerplate in each component.
...
2.  Looping over that array to generate a link for each one to the `Price` route.
3.  Placing the currency symbol in the `:symbol` part of the URL.
...
1.  Store the API key and currency symbol in different variables.
...
5.  Include a `loaded` and `loading` function for rendering the data if exists.
